...
public class DragDiootoView extends FrameLayout {
    ...
    public DragDiootoView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (!ImmersionBar.hasNotchScreen((Activity) getContext())) {
            //没有刘海屏
            screenHeight = getResources().getDisplayMetrics().heightPixels;
        } else {
            //获取包含导航栏在内的的高度。在imageActivity里。对其导航栏进行透明化
            screenHeight = getHasVirtualKey();
//            Log.e("包含导航栏的高度", getHasVirtualKey() + "");
//            screenHeight = getResources().getDisplayMetrics().heightPixels + ImmersionBar.getNotchHeight((Activity) getContext());
        }
...
//        }
    ...
    public void show(boolean showImmediately) {
        Log.e("我就不信该部了", showImmediately + " 00");
    ...
    public boolean isLikeWx() {
        return isLikeWx;
    }

    public void setLikeWx(boolean likeWx) {
        isLikeWx = likeWx;
    }
}
